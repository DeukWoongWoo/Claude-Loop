...
setup() {
    ...
    SCRIPT_PATH="$BATS_TEST_DIRNAME/../claude_loop.sh"
    export TESTING="true"
}
...
@test "show_version displays version" {
    ...
    run show_version
    assert_output --partial "claude-loop version"
}
...
@test "get_script_path returns script path" {
    ...
    function readlink() {
        if [ "$1" = "-f" ]; then
            echo "/usr/local/bin/claude-loop"
            return 0
        fi
        return 1
    }
    ...
    assert_success
    assert_output "/usr/local/bin/claude-loop"
}
    ...
    function curl() {
        ...
        if [ -n "$output_file" ]; then
            ...
            if [[ "$output_file" == *".sha256"* ]] || [[ "${@}" == *".sha256"* ]]; then
                ...
                echo "dummychecksum123456789  claude_loop.sh" > "$output_file"
            else
...
@test "check_for_updates with skip_prompt does not prompt" {
    ...
    assert_success
    assert_output --partial "A new version of claude-loop is available"
...
@test "claude_loop_commit dry run shows PR merged message with placeholder" {
    source "$SCRIPT_PATH"
    ...
    run claude_loop_commit "(1/1)" "test-branch" "main"
...
# ============================================================================
# PRINCIPLES FRAMEWORK TESTS
# ============================================================================

@test "parse_arguments accepts --reset-principles flag" {
    source "$SCRIPT_PATH"

    parse_arguments --reset-principles -p "test" -m 1

    assert_equal "$RESET_PRINCIPLES" "true"
}

@test "parse_arguments accepts --principles-file with path" {
    source "$SCRIPT_PATH"

    parse_arguments --principles-file "/custom/path.yaml" -p "test" -m 1

    assert_equal "$PRINCIPLES_FILE" "/custom/path.yaml"
}

@test "parse_arguments accepts --log-decisions flag" {
    source "$SCRIPT_PATH"

    parse_arguments --log-decisions -p "test" -m 1

    assert_equal "$LOG_DECISIONS" "true"
}

@test "generate_startup_preset outputs valid YAML with correct preset" {
    source "$SCRIPT_PATH"

    run generate_startup_preset

    assert_success
    assert_output --partial "preset: startup"
    assert_output --partial "scope_philosophy: 3"
    assert_output --partial "speed_correctness: 4"
}

@test "generate_enterprise_preset outputs valid YAML with correct preset" {
    source "$SCRIPT_PATH"

    run generate_enterprise_preset

    assert_success
    assert_output --partial "preset: enterprise"
    assert_output --partial "scope_philosophy: 5"
    assert_output --partial "speed_correctness: 8"
}

@test "generate_opensource_preset outputs valid YAML with correct preset" {
    source "$SCRIPT_PATH"

    run generate_opensource_preset

    assert_success
    assert_output --partial "preset: opensource"
    assert_output --partial "scope_philosophy: 2"
    assert_output --partial "speed_correctness: 8"
}

@test "generate_default_principles creates file with startup preset" {
    source "$SCRIPT_PATH"

    local temp_dir=$(mktemp -d)
    PRINCIPLES_FILE="$temp_dir/.claude/principles.yaml"

    run generate_default_principles startup

    assert_success
    assert [ -f "$PRINCIPLES_FILE" ]

    # Verify content
    run cat "$PRINCIPLES_FILE"
    assert_output --partial "preset: startup"

    rm -rf "$temp_dir"
}

@test "generate_default_principles applies overrides correctly" {
    source "$SCRIPT_PATH"

    local temp_dir=$(mktemp -d)
    PRINCIPLES_FILE="$temp_dir/.claude/principles.yaml"

    run generate_default_principles startup scope_philosophy=7 speed_correctness=8

    assert_success

    # Verify overrides were applied
    run cat "$PRINCIPLES_FILE"
    assert_output --partial "scope_philosophy: 7"
    assert_output --partial "speed_correctness: 8"

    rm -rf "$temp_dir"
}

@test "generate_default_principles fails for unknown preset" {
    source "$SCRIPT_PATH"

    run generate_default_principles invalid_preset

    assert_failure
    assert_output --partial "Unknown preset"
}

@test "load_principles loads valid YAML file and extracts preset" {
    source "$SCRIPT_PATH"
...
EOF
    ...
    assert_success
    assert_output --partial "Principles loaded [startup]"

    rm -rf "$temp_dir"
}

@test "load_principles fails for missing file" {
    source "$SCRIPT_PATH"

    PRINCIPLES_FILE="/nonexistent/path/principles.yaml"

    run load_principles

    assert_failure
    assert_output --partial "Principles file not found"
}

@test "detect_principle_conflict returns 0 for PRINCIPLE_CONFLICT_UNRESOLVED" {
    source "$SCRIPT_PATH"

    run detect_principle_conflict "Some text PRINCIPLE_CONFLICT_UNRESOLVED more text"

    assert_success
}

@test "detect_principle_conflict returns 0 for cannot resolve principle" {
    source "$SCRIPT_PATH"

    run detect_principle_conflict "I cannot resolve this principle conflict"

    assert_success
}

@test "detect_principle_conflict returns 1 for normal responses" {
    source "$SCRIPT_PATH"

    run detect_principle_conflict "Task completed successfully. All tests pass."

    assert_failure
}

@test "log_principle_decision does nothing when LOG_DECISIONS is false" {
    source "$SCRIPT_PATH"
    ...
    cd "$temp_dir"
    run log_principle_decision 1 "**Decision**: Test" false

    assert_success
    assert [ ! -f ".claude/principles-decisions.log" ]

    rm -rf "$temp_dir"
}

@test "log_principle_decision creates log when LOG_DECISIONS is true" {
    source "$SCRIPT_PATH"

    local temp_dir=$(mktemp -d)
    LOG_DECISIONS=true
    PRINCIPLE_PRESET=startup

    cd "$temp_dir"
    run log_principle_decision 1 "**Decision**: Test decision **Rationale**: Testing" false

    assert_success
    assert [ -f ".claude/principles-decisions.log" ]

    run cat ".claude/principles-decisions.log"
